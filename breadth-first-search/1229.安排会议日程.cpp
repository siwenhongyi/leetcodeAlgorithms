...
public:
	vector<int> minAvailableDuration(vector<vector<int>>& s, vector<vector<int>>& t, int duration) {
		...
		i = j = 0;
		int p, pp;
		sort(s.begin(), s.end(), [](const vector<int>& q, const vector<int>& qq) {if (q[0] != qq[0])return q[0] < qq[0]; return q[1] < qq[1]; });
		sort(t.begin(), t.end(), [](const vector<int>& q, const vector<int>& qq) {if (q[0] != qq[0])return q[0] < qq[0]; return q[1] < qq[1]; });
		while (i < m && j < n) {
