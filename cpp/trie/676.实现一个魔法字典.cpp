...
class MagicDictionary {
 private:
  ...
  bool dfs(string& word, int index, bool t, letter* root) {
    if (index == word.size() && !t) return root->is_word;
    char ch = word[index];
    if (root->nexts.count(ch) && dfs(word, index + 1, t, root->nexts[ch]))
      return true;
    if (t) {
      for (auto iter : root->nexts) {
        if (iter.first == ch) continue;
        if (dfs(word, index + 1, false, iter.second)) return true;
      }
    }
    return false;
  }
 ...
 public:
  ...
  MagicDictionary() { root = new letter; }
  ...
  void buildDict(vector<string> dict) {
    for (auto& iter : dict) {
      auto p = root;
      for (char ch : iter) {
        if (!p->nexts.count(ch)) p->nexts[ch] = new letter;
        p = p->nexts[ch];
      }
      p->is_word = true;
    }
  ...
  bool search(string word) { return dfs(word, 0, true, root); }
};
