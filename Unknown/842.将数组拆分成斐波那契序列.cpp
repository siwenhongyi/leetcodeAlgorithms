...
public:
	vector<int> splitIntoFibonacci(string S) {
		...
		if (S[0] == '0') {
			...
			for (int lenTwo = 1; lenTwo < strSize - 1 && lenTwo < 11; ++lenTwo) {
                //注意0 <= F[i] <= 2^31 - 1，（也就是说，每个整数都符合 32 位有符号整数类型），元素最大长度为10
			...
			for (int lenOne = 1; lenOne < strSize - 1 && lenOne < 11; ++lenOne) {
                ...
                if (tempValOne < INT_MAX){
                    ...
                    if (S[lenOne] == '0') {
                        ...
                        for (int lenTwo = 1; lenTwo < strSize - lenOne && lenTwo < 11; ++lenTwo) {
                            //穷举切割第二个元素
	...
	bool helper(string &S, vector<int> &tempRes, int index) {
		if (index == S.size()) {
			...
			if (S.substr(index, nextStr.size()) == nextStr && helper(S, tempRes, index + nextStr.size())) {
				return true;
			}
			tempRes.pop_back();
			return false;
